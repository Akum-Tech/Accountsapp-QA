...
import { decreptionCredit, encreptionCredit } from '../security/creditvoucher';
...
exports.getSingleData = async function (id, data, res) {
    try {
        ...
        if(createdata){
            let response = await decreptionCredit(createdata, 'object', data.data.email);
...
exports.getAllData = async function (data, res) {
    try {
        let createdata = await CreditVoucher.findAll({
            where: {
                company_id: data.company_id,
                current_year: data.current_year,
                end_year: data.end_year,
        ...
        if(createdata.length>0){
            let response = await decreptionCredit(createdata, 'array', data.data.email);
...
exports.getAllDataPagination = async function (data, res) {
    try {
        let createdata = await CreditVoucher.findAndCountAll({
             limit: data.limit,
            offset: data.offset,
            where: {
                company_id: data.company_id,
        ...
        if(createdata.rows.length>0){
            let response = await decreptionCredit(createdata.rows, 'array', data.data.email);
            response = response.map(item=>{
                if(item.invoice_id){
                    item.invoice_id = item.invoice_id<=9?`${data.current_year.toString().substr(-2)+`-`+data.end_year.toString().substr(-2)}/00${item.invoice_id}`:item.invoice_id>9?`${data.current_year.toString().substr(-2)+`-`+data.end_year.toString().substr(-2)}/0${item.invoice_id}`:`${data.current_year.toString().substr(-2)+`-`+data.end_year.toString().substr(-2)}/${item.invoice_id}`;
                }
...
exports.createData = async function (data, res) {
    try {
        const result = await sequelize.transaction(async(t) => {
           if(data.is_after){
              let count = await CreditVoucher.findOne({where:{[Op.and]: [ {uid:data.after_id},{company_id:data.company_id}]}}, t);
              let checkYear = await CreditVoucher.findOne({where:{[Op.and]: [{current_year: data.current_year },{end_year: data.end_year },{company_id:data.company_id}]}}, t)
              if(checkYear){
                data.invoice_id = await Number(count.dataValues.invoice_id)+1;
                let updateCount = await CreditVoucher.update({invoice_id : Sequelize.literal('invoice_id+1')}, {where:{[Op.and]: [{current_year: data.current_year },{end_year: data.end_year },{company_id:data.company_id}],invoice_id:{[Op.gte]:data.invoice_id}}}, t)
              }else{
                data.invoice_id = await 1;
              }
            }else if(data.is_before){
                let count = await CreditVoucher.findOne({where:{[Op.and]: [ {invoice_date:{[Op.lte]:data.invoice_date}},{current_year: data.current_year },{end_year: data.end_year },{company_id:data.company_id}]}, order: [ [ 'invoice_id', 'DESC' ]]}, t);
                let checkYear = await CreditVoucher.findOne({where:{[Op.and]: [{current_year: data.current_year },{end_year: data.end_year },{company_id:data.company_id}]}}, t)
                if(checkYear){
                    // console.log("count", count.dataValues);
                    let finddate = await CreditVoucher.findOne({where:{[Op.and]: [{invoice_date:data.invoice_date},{current_year: data.current_year },{end_year: data.end_year },{company_id:data.company_id}]}, order: [ [ 'invoice_id', 'DESC' ]]}, t);
                    // console.log("finddate", finddate, data.invoice_id)
                    if(finddate){
                        data.invoice_id = await Number(finddate.dataValues.invoice_id)+1; 
                        let updateCount = await CreditVoucher.update({invoice_id : Sequelize.literal('invoice_id+1')}, {where:{[Op.and]: [{current_year: data.current_year },{end_year: data.end_year },{company_id:data.company_id},{invoice_id:{[Op.gte]:data.invoice_id}}]}, order: [ [ 'invoice_id', 'DESC' ]]}, t);
                    }else{
                        if(count){
                            data.invoice_id = await Number(count.dataValues.invoice_id)+1;
                        }else{
                        ...
                        let updateCount = await CreditVoucher.update({invoice_id : Sequelize.literal('invoice_id+1')}, {where:{[Op.and]: [{current_year: data.current_year },{end_year: data.end_year },{company_id:data.company_id},{invoice_date:{[Op.gte]:data.invoice_date}}]}}, t)
                    }
                }else{
                    data.invoice_id = await 1;
                }
                ...
                let count  = await CreditVoucher.count({where:{[Op.and]: [{current_year: data.current_year },{end_year: data.end_year },{company_id:data.company_id}]}});
                let checkYear = await CreditVoucher.findOne({where:{[Op.and]: [{current_year: data.current_year },{end_year: data.end_year },{company_id:data.company_id}]}}, t)
                if(checkYear){
            ...
            let createdata = await CreditVoucher.create(data, {transaction:t});
            entry.createData(data.company_id, Message.credit_note_create);
...
exports.deleteData = async function (id, data, res) {
    try {
        let find =await CreditVoucher.findOne({where:{uid:id}});
        if(find){
            let updateCount = await CreditVoucher.update({invoice_id : Sequelize.literal('invoice_id-1')}, {where:{[Op.and]: [{current_year: find.dataValues.current_year },{end_year: find.dataValues.end_year },{company_id:find.dataValues.company_id}],invoice_id:{[Op.gte]:Number(find.dataValues.invoice_id)}}})
            ...
            let deletedata = await  CreditVoucher.destroy({ where: {uid:id}});
            if(deletedata){
                return {
                    statusCode:res.statusCode,
                    success: true,
    ...
    try {
        const result = await sequelize.transaction(async(t) => {
            ...
            // }
             ...
             let companydata = await Company.findOne({where:{uid:data.company_id}},t);
             let updatedata = await CreditVoucher.findOne({where:{uid:data.uid}},t);
             if(updatedata){
                delete data.invoice_id;
                await updatedata.update(data);
                await entry.createData(data.company_id, Message.credit_note_update);
                if(itemData.length>0){
             ...
             let response = await decreptionCredit(updatedata, 'object', data.data.email);
             return response;
         });
         if(result){
             return {
...
exports.cancelData = async function (id, data, res) {
    try {
        const result = await sequelize.transaction(async(t) => {
             ...
             data = await encreptionCredit(data, data.data.email)
             let updatedata = await CreditVoucher.findOne({where:{uid:data.uid}},t);
             if(updatedata){
               delete data.invoice_id;
                await updatedata.update(data);
